    ...
    IUnitOfWork unitOfWork) : INotificationService
{
    ...
    public async Task SendDigestAsync(Guid authorId, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            await emailSender.SendAsync(
                ...
                "ScrivenerSync - New comments digest", body, ct);
    ...
    private static string GetSubject(EmailType emailType) => emailType switch
    {
        ...
        EmailType.CommentNotification       => "ScrivenerSync - New comment on your manuscript",
        EmailType.ReplyNotification         => "ScrivenerSync - New reply to your comment",
        EmailType.NewSectionNotification    => "ScrivenerSync - New section available to read",
        EmailType.SectionChangedNotification => "ScrivenerSync - A section has been updated",
        EmailType.DigestNotification        => "ScrivenerSync - New comments digest",
        _                                   => "ScrivenerSync notification"
    };
    ...
    private static string GetBody(EmailType emailType, Guid? relatedEntityId) => emailType switch
    {
        ...
        _                                => "You have a new notification on ScrivenerSync."
    };
}

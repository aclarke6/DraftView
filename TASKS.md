# DraftView ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Task List
...
- 1 skipped ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â `SmtpEmailSenderIntegrationTests` (sends real email, manual only)

### Active Work
| Track | Status |
|-------|--------|
| RSprint Series | ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¡ In progress ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â RS-A to RS-E complete, RS-F next |
| S-Sprint Series | ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¡ In progress ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â S-Sprint-1 complete, S-Sprint-2 next |
| MT-Sprint Series | ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â´ HIGH PRIORITY ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â second author interest accelerates timeline; see `MultiTenancy.md` |
| RD-Sprint Series | ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¡ In progress ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â RD-Sprint-1 (Continue Reading CTA) merged to main 2026-08-17; see section 3.7 |
| DR-Sprint Series | ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Complete ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â merged to main 2026-08-17; see section 3.8 |
| Go-Live Prerequisites | ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â´ Blocking ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â items below must complete before launch |
| UAT | ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¡ In progress |
...
| Document | Purpose |
|----------|---------|
| `AGENTS.md` | Authoritative execution rules for all coding agents ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â defines constraints, architecture boundaries, TDD requirements, and hard-gated response behaviour across all tools |
...
| `DropBox Synchronisation Using WebHooks.md` | Webhook-driven background Dropbox sync ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â control model, cursor-based interrogation, and S-Sprint series |
| `MultiTenancy.md` | Multi-tenancy sprint series, design decisions, and migration strategy |
| `Publishing And Versioning Architecture.md` | Versioning model ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â SectionVersion, publish/republish rules, and lifecycle behaviour |
| `DraftView-UAT-Plan.md` | UAT plan and validation scenarios for reader and author workflows |
| `PRINCIPLES.md` | Core engineering principles ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â architecture, layering, and behavioural rules |
...
- [SUPERSEDED] CHANGE-003 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â replaced by CHANGE-006 and CHANGE-007 below

- [ ] CHANGE-006 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Reader left nav: collapsible act tree + panel pin/unpin
  ...
  **Nav tree structure:**
  ...
  - Each intermediate folder (Act, Part, or whatever the author named it) becomes a collapsible row, labelled with the author's own title, with a `ÃƒÂ¢Ã¢â€šÂ¬Ã‚Âº` chevron toggle
  - The act containing the current chapter starts expanded; all others start collapsed
  - "Other Chapters" heading and section are removed ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â all chapter navigation lives inside the act tree
  - "This Chapter" scene list remains at the top, unchanged

  **Data model ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no changes needed:**
  ...
  **Panel pin/unpin behaviour:**
  - Desktop (ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¥ 992 px): panel starts pinned open
  - Mobile/tablet: panel starts collapsed; a `ÃƒÂ¢Ã‹Å“Ã‚Â°` button opens it as an overlay
  - Unpinned: panel auto-collapses after a chapter link is clicked
  - Pinned: panel stays open after selection
  - A pin icon (ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã…â€™ / thumb-tack SVG) toggles pin state
  - Pin state and per-act collapsed state persisted in `localStorage`

  **Files affected:**
  - `DraftView.Web/Views/Reader/DesktopRead.cshtml` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â replace static headings with `<button>` toggles; add pin button; remove "Other Chapters" block
  - `DraftView.Web/wwwroot/css/DraftView.Reader.css` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â panel slide transition, collapsed state, pin icon, overlay backdrop
  - `DraftView.Web/wwwroot/js/reader-nav.js` *(new)* ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â toggle logic, pin state, localStorage, auto-collapse on nav

- [DONE] CHANGE-006 + CHANGE-007 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Collapsible reader nav and comments toggle ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â merged to main 2026-08-15. See `HISTORY.md`.

- [x] CHANGE-008 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Mobile reader: chapter comments page + disable passage-anchor capture on touch ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â merged 2026-08-16. See `HISTORY.md`.
- [x] CHANGE-009 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Mobile reader: read-first scene comments ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â merged 2026-08-16. See `HISTORY.md`.

---

## 3. Active Projects

### 3.1 RSprint ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Passage Anchoring, Reader Continuity, and Inline Commentary

**Status:** ÃƒÂ°Ã…Â¸Ã…Â¸Ã‚Â¡ In progress ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â RS-A through RS-E complete (see `HISTORY.md`), RS-F next

- [ ] **RS-F ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Original Context**
  - [ ] Phase F1 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Retrieve original version content
  - [ ] Phase F2 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Navigate to original anchor
  - [ ] Phase F3 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â UI integration ("View original context")

- [REMOVED] **RS-G ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â AI-Assisted Relocation** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â removed 2026-08-17.
  ...
  highlighted passage. This is the confirmed fallback behaviour ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no AI required.

- [ ] **RS-H ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Reader Insight**
  - [ ] Phase H1 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Progress tracking (anchor-based)
  - [ ] Phase H2 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Author insight (reader activity)
  - [ ] Phase H3 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â UI (drill-down and indicators)
...
- [x] Apply pending EF migrations to production DB Ã¢â‚¬â€ all 27 migrations applied and in sync (verified 2026-08-17)
- [ ] UAT: complete scenarios CÃ¢â‚¬â€œK (A, B partial, H passed Ã¢â‚¬â€ see `DraftView-UAT-Plan.md`)
...
- [x] Fail2ban setup on production VM (verified active 2026-08-17 Ã¢â‚¬â€ sshd jail, 44 IPs banned since launch)
...
- [x] CHANGE-010 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Impersonation ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â read-only reader view (implemented on change/CHANGE-010-reader-impersonation)

---

### 3.4 Multi-Tenancy Sprint Series

**ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â´ HIGH PRIORITY** ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â A second author has expressed interest in the platform. Readers may read
...
S-Sprint-1 complete ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see `HISTORY.md`.

- [ ] **S-Sprint-2 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Webhook receipt and durable request recording**
...
- [ ] **S-Sprint-3 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Immediate orchestration path**
...
- [ ] **S-Sprint-4 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Dropbox delta interrogation and incremental download**
...
- [ ] **S-Sprint-5 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Reuse existing sync pipeline end to end**
...
- [ ] **S-Sprint-6 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Periodic worker and held request recovery**
...
- [ ] **S-Sprint-7 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Stale reconciliation and operational hardening**
...
- [ ] **S-Sprint-8 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Daily health check and reconciliation app**
...
See `REFACTORING.md` for full detail. Phase 1 complete ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â see `HISTORY.md`.

- [ ] Phase 2 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Extract procedural controller workflows
- [ ] Phase 3 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Decompose startup/seeding
- [ ] Phase 4 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Standardise inheritance and shared utilities
- [ ] Phase 5 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Extract remaining procedural workflows
- [ ] Phase 6 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Standardise sync kickoff (remove inline `Task.Run`)

---

### 3.7 RD-Sprint ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Reader Dashboard

**Status:** ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Âµ Pre-planning
...
- `Role.Author` ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `/Author/Dashboard` (existing)
- `Role.BetaReader` ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `/Reader/Dashboard` (new cold landing)
...
1. **Comment Replies** *(conditional hero ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â only shown when unread replies exist)*
   ...
   - When empty: section is hidden entirely ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â book list takes the full page
...
4. **Discover Authors** *(link ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ `/Reader/Discovery`)*
...
**Infrastructure needed (new ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â not yet built):**
- `ICommentRepository.GetRepliesToUserCommentsAsync(Guid userId)` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â joins reply ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ parent,
...
- `IProjectRepository.GetAllForReaderAsync(Guid userId)` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â cross-project reader query
...
- Home controller role-based redirect: `/` ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ author or reader dashboard by role

**Assets:**
- `DraftViewReaderDash.Hero.png` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â hero image for the dashboard cold landing. Added to main 2026-08-16.
...
| RD-Sprint-3 | ~~Discover Authors page~~ ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â superseded by DR-Sprint (complete 2026-08-17) |

---

### 3.8 DR-Sprint ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Open Book Discovery & Access Requests

**Status:** ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Complete ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â all 5 phases merged to main 2026-08-17. See `HISTORY.md` for full detail.
...
- Ubuntu OS upgrade: 20.04 Ã¢â€ â€™ 22.04 Ã¢â€ â€™ 24.04 via `do-release-upgrade` from Oracle Cloud console (pg_dump first; keep current config files when prompted; verify all services after each hop)

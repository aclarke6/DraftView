...
---
mode: agent
description: S-Sprint-3 Phase 2 - Cooldown hold evaluation
---

# S-Sprint-3 Phase 2 - Cooldown hold evaluation

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-3-base` from `main` if it does not already exist
3. Create `S-Sprint-3-base/phase-2-cooldown-hold-evaluation` from `S-Sprint-3-base`
4. All work on `S-Sprint-3-base/phase-2-cooldown-hold-evaluation`
5. Developer merges: `S-Sprint-3-base/phase-2-cooldown-hold-evaluation` -> `S-Sprint-3-base` -> `main`

## Context
This is Phase 2 of S-Sprint-3 (Immediate Orchestration Path).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-3 for full phase architecture.

**Phase Brief:** Add the control logic that prevents immediate sync when HeldUntilUtc is still in force while preserving outstanding demand.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-3 Phase 2 section
...
### Phase Goal
Add the control logic that prevents immediate sync when HeldUntilUtc is still in force while preserving outstanding demand.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-3 Phase 2 for:
...
- [ ] Committed to `S-Sprint-3-base/phase-2-cooldown-hold-evaluation` with message:
    `feat: S-Sprint-3 Phase 2 - Cooldown hold evaluation`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-3 Phase 2 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-3 Phase 2 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-3-base
  git merge S-Sprint-3-base/phase-2-cooldown-hold-evaluation
  git checkout main

...
---
mode: agent
description: S-Sprint-2 Phase 3 - Request recording service
---

# S-Sprint-2 Phase 3 - Request recording service

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-2-base` from `main` if it does not already exist
3. Create `S-Sprint-2-base/phase-3-request-recording-service` from `S-Sprint-2-base`
4. All work on `S-Sprint-2-base/phase-3-request-recording-service`
5. Developer merges: `S-Sprint-2-base/phase-3-request-recording-service` -> `S-Sprint-2-base` -> `main`

## Context
This is Phase 3 of S-Sprint-2 (Webhook Receipt and Durable Request Recording).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-2 for full phase architecture.

**Phase Brief:** Introduce an application service that records SyncRequestedUtc and LastWebhookUtc for all ScrivenerDropbox projects when a webhook is received (single-author mode).

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-2 Phase 3 section
...
### Phase Goal
Introduce an application service that records SyncRequestedUtc and LastWebhookUtc for all ScrivenerDropbox projects when a webhook is received (single-author mode).

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-2 Phase 3 for:
...
- [ ] Committed to `S-Sprint-2-base/phase-3-request-recording-service` with message:
    `feat: S-Sprint-2 Phase 3 - Request recording service`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-2 Phase 3 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-2 Phase 3 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-2-base
  git merge S-Sprint-2-base/phase-3-request-recording-service
  git checkout main

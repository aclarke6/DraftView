...
---
mode: agent
description: S-Sprint-4 Phase 4 - Dropbox delta tests
---

# S-Sprint-4 Phase 4 - Dropbox delta tests

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-4-base` from `main` if it does not already exist
3. Create `S-Sprint-4-base/phase-4-dropbox-delta-tests` from `S-Sprint-4-base`
4. All work on `S-Sprint-4-base/phase-4-dropbox-delta-tests`
5. Developer merges: `S-Sprint-4-base/phase-4-dropbox-delta-tests` -> `S-Sprint-4-base` -> `main`

## Context
This is Phase 4 of S-Sprint-4 (Dropbox Delta Interrogation and Incremental Download).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-4 for full phase architecture.

**Phase Brief:** Add tests proving cursor progression, relevant-path filtering, and changed-entry download behaviour without changing publishing rules.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-4 Phase 4 section
...
### Phase Goal
Add tests proving cursor progression, relevant-path filtering, and changed-entry download behaviour without changing publishing rules.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-4 Phase 4 for:
...
- [ ] Committed to `S-Sprint-4-base/phase-4-dropbox-delta-tests` with message:
    `feat: S-Sprint-4 Phase 4 - Dropbox delta tests`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-4 Phase 4 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-4 Phase 4 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-4-base
  git merge S-Sprint-4-base/phase-4-dropbox-delta-tests
  git checkout main

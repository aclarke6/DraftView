...
---
mode: agent
description: S-Sprint-2 Phase 2 - Signature validation and request parsing
---

# S-Sprint-2 Phase 2 - Signature validation and request parsing

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-2-base` from `main` if it does not already exist
3. Create `S-Sprint-2-base/phase-2-signature-validation-and-request-parsing` from `S-Sprint-2-base`
4. All work on `S-Sprint-2-base/phase-2-signature-validation-and-request-parsing`
5. Developer merges: `S-Sprint-2-base/phase-2-signature-validation-and-request-parsing` -> `S-Sprint-2-base` -> `main`

## Context
This is Phase 2 of S-Sprint-2 (Webhook Receipt and Durable Request Recording).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-2 for full phase architecture.

**Phase Brief:** Implement Dropbox webhook signature validation and parsing of changed account identifiers, keeping the controller or endpoint thin.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-2 Phase 2 section
...
### Phase Goal
Implement Dropbox webhook signature validation and parsing of changed account identifiers, keeping the controller or endpoint thin.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-2 Phase 2 for:
...
- [ ] Committed to `S-Sprint-2-base/phase-2-signature-validation-and-request-parsing` with message:
    `feat: S-Sprint-2 Phase 2 - Signature validation and request parsing`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-2 Phase 2 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-2 Phase 2 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-2-base
  git merge S-Sprint-2-base/phase-2-signature-validation-and-request-parsing
  git checkout main
  git merge S-Sprint-2-base

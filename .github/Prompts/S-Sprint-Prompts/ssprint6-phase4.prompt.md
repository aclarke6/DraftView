...
---
mode: agent
description: S-Sprint-6 Phase 4 - Worker tests
---

# S-Sprint-6 Phase 4 - Worker tests

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-6-base` from `main` if it does not already exist
3. Create `S-Sprint-6-base/phase-4-worker-tests` from `S-Sprint-6-base`
4. All work on `S-Sprint-6-base/phase-4-worker-tests`
5. Developer merges: `S-Sprint-6-base/phase-4-worker-tests` -> `S-Sprint-6-base` -> `main`

## Context
This is Phase 4 of S-Sprint-6 (Periodic Worker and Held Request Recovery).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-6 for full phase architecture.

**Phase Brief:** Add tests proving worker eligibility rules, bounded batch behaviour, held request recovery, and respect for active leases.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-6 Phase 4 section
...
### Phase Goal
Add tests proving worker eligibility rules, bounded batch behaviour, held request recovery, and respect for active leases.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-6 Phase 4 for:
...
- [ ] Committed to `S-Sprint-6-base/phase-4-worker-tests` with message:
    `feat: S-Sprint-6 Phase 4 - Worker tests`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-6 Phase 4 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-6 Phase 4 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-6-base
  git merge S-Sprint-6-base/phase-4-worker-tests
  git checkout main

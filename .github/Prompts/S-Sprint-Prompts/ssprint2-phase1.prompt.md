...
---
mode: agent
description: S-Sprint-2 Phase 1 - Webhook endpoint surface
---

# S-Sprint-2 Phase 1 - Webhook endpoint surface

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-2-base` from `main` if it does not already exist
3. Create `S-Sprint-2-base/phase-1-webhook-endpoint-surface` from `S-Sprint-2-base`
4. All work on `S-Sprint-2-base/phase-1-webhook-endpoint-surface`
5. Developer merges: `S-Sprint-2-base/phase-1-webhook-endpoint-surface` -> `S-Sprint-2-base` -> `main`

## Context
This is Phase 1 of S-Sprint-2 (Webhook Receipt and Durable Request Recording).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-2 for full phase architecture.

**Phase Brief:** Add a single Dropbox webhook endpoint in the existing web application with handshake support and request validation scaffolding.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-2 Phase 1 section
...
### Phase Goal
Add a single Dropbox webhook endpoint in the existing web application with handshake support and request validation scaffolding.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-2 Phase 1 for:
...
- [ ] Committed to `S-Sprint-2-base/phase-1-webhook-endpoint-surface` with message:
    `feat: S-Sprint-2 Phase 1 - Webhook endpoint surface`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-2 Phase 1 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-2 Phase 1 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-2-base
  git merge S-Sprint-2-base/phase-1-webhook-endpoint-surface
  git checkout main

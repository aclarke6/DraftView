...
---
mode: agent
description: S-Sprint-6 Phase 3 - Held request recovery
---

# S-Sprint-6 Phase 3 - Held request recovery

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-6-base` from `main` if it does not already exist
3. Create `S-Sprint-6-base/phase-3-held-request-recovery` from `S-Sprint-6-base`
4. All work on `S-Sprint-6-base/phase-3-held-request-recovery`
5. Developer merges: `S-Sprint-6-base/phase-3-held-request-recovery` -> `S-Sprint-6-base` -> `main`

## Context
This is Phase 3 of S-Sprint-6 (Periodic Worker and Held Request Recovery).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-6 for full phase architecture.

**Phase Brief:** Ensure that requests deferred by HeldUntilUtc are picked up automatically once eligible.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-6 Phase 3 section
...
### Phase Goal
Ensure that requests deferred by HeldUntilUtc are picked up automatically once eligible.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-6 Phase 3 for:
...
- [ ] Committed to `S-Sprint-6-base/phase-3-held-request-recovery` with message:
    `feat: S-Sprint-6 Phase 3 - Held request recovery`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-6 Phase 3 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-6 Phase 3 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-6-base
  git merge S-Sprint-6-base/phase-3-held-request-recovery
  git checkout main
  git merge S-Sprint-6-base

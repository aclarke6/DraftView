...
---
mode: agent
description: S-Sprint-5 Phase 1 - Existing pipeline integration seam
---

# S-Sprint-5 Phase 1 - Existing pipeline integration seam

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-5-base` from `main` if it does not already exist
3. Create `S-Sprint-5-base/phase-1-existing-pipeline-integration-seam` from `S-Sprint-5-base`
4. All work on `S-Sprint-5-base/phase-1-existing-pipeline-integration-seam`
5. Developer merges: `S-Sprint-5-base/phase-1-existing-pipeline-integration-seam` -> `S-Sprint-5-base` -> `main`

## Context
This is Phase 1 of S-Sprint-5 (Reuse Existing Sync Pipeline End to End).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-5 for full phase architecture.

**Phase Brief:** Introduce the application seam that allows the background orchestrator to invoke the current sync pipeline without duplicating parsing or working-state update logic.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-5 Phase 1 section
...
### Phase Goal
Introduce the application seam that allows the background orchestrator to invoke the current sync pipeline without duplicating parsing or working-state update logic.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-5 Phase 1 for:
...
- [ ] Committed to `S-Sprint-5-base/phase-1-existing-pipeline-integration-seam` with message:
    `feat: S-Sprint-5 Phase 1 - Existing pipeline integration seam`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-5 Phase 1 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-5 Phase 1 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-5-base
  git merge S-Sprint-5-base/phase-1-existing-pipeline-integration-seam
  git checkout main

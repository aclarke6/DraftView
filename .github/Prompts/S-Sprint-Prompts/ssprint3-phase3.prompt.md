...
---
mode: agent
description: S-Sprint-3 Phase 3 - Background sync orchestration service shell
---

# S-Sprint-3 Phase 3 - Background sync orchestration service shell

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-3-base` from `main` if it does not already exist
3. Create `S-Sprint-3-base/phase-3-background-sync-orchestration-service-shell` from `S-Sprint-3-base`
4. All work on `S-Sprint-3-base/phase-3-background-sync-orchestration-service-shell`
5. Developer merges: `S-Sprint-3-base/phase-3-background-sync-orchestration-service-shell` -> `S-Sprint-3-base` -> `main`

## Context
This is Phase 3 of S-Sprint-3 (Immediate Orchestration Path).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-3 for full phase architecture.

**Phase Brief:** Create the orchestration service that loads control state, checks hold and lease rules, and decides whether to exit or proceed.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-3 Phase 3 section
...
### Phase Goal
Create the orchestration service that loads control state, checks hold and lease rules, and decides whether to exit or proceed.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-3 Phase 3 for:
...
- [ ] Committed to `S-Sprint-3-base/phase-3-background-sync-orchestration-service-shell` with message:
    `feat: S-Sprint-3 Phase 3 - Background sync orchestration service shell`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-3 Phase 3 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-3 Phase 3 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-3-base
  git merge S-Sprint-3-base/phase-3-background-sync-orchestration-service-shell
  git checkout main

...
---
mode: agent
description: S-Sprint-4 Phase 3 - Incremental download integration
---

# S-Sprint-4 Phase 3 - Incremental download integration

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-4-base` from `main` if it does not already exist
3. Create `S-Sprint-4-base/phase-3-incremental-download-integration` from `S-Sprint-4-base`
4. All work on `S-Sprint-4-base/phase-3-incremental-download-integration`
5. Developer merges: `S-Sprint-4-base/phase-3-incremental-download-integration` -> `S-Sprint-4-base` -> `main`

## Context
This is Phase 3 of S-Sprint-4 (Dropbox Delta Interrogation and Incremental Download).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-4 for full phase architecture.

**Phase Brief:** Reuse the existing changed-entry download path to fetch only relevant changed files into the existing local cache structure.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-4 Phase 3 section
...
### Phase Goal
Reuse the existing changed-entry download path to fetch only relevant changed files into the existing local cache structure.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-4 Phase 3 for:
...
- [ ] Committed to `S-Sprint-4-base/phase-3-incremental-download-integration` with message:
    `feat: S-Sprint-4 Phase 3 - Incremental download integration`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-4 Phase 3 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-4 Phase 3 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-4-base
  git merge S-Sprint-4-base/phase-3-incremental-download-integration
  git checkout main
  git merge S-Sprint-4-base

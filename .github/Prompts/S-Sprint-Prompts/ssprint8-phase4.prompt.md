...
---
mode: agent
description: S-Sprint-8 Phase 4 - Full rescan orchestration and operational verification
---

# S-Sprint-8 Phase 4 - Full rescan orchestration and operational verification

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-8-base` from `main` if it does not already exist
3. Create `S-Sprint-8-base/phase-4-full-rescan-orchestration-and-operational-verification` from `S-Sprint-8-base`
4. All work on `S-Sprint-8-base/phase-4-full-rescan-orchestration-and-operational-verification`
5. Developer merges: `S-Sprint-8-base/phase-4-full-rescan-orchestration-and-operational-verification` -> `S-Sprint-8-base` -> `main`

## Context
This is Phase 4 of S-Sprint-8 (Daily Health Check and Reconciliation App).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-8 for full phase architecture.

**Phase Brief:** Implement full rescan path (query all Dropbox entries without cursor, download only changed files), test on projects with broken cursors, verify no version creation or publishing, and confirm lease mutual exclusion with webhook workers.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-8 Phase 4 section
...
### Phase Goal
Implement full rescan path (query all Dropbox entries without cursor, download only changed files), test on projects with broken cursors, verify no version creation or publishing, and confirm lease mutual exclusion with webhook workers.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-8 Phase 4 for:
...
- [ ] Committed to `S-Sprint-8-base/phase-4-full-rescan-orchestration-and-operational-verification` with message:
    `feat: S-Sprint-8 Phase 4 - Full rescan orchestration and operational verification`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-8 Phase 4 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-8 Phase 4 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-8-base
  git merge S-Sprint-8-base/phase-4-full-rescan-orchestration-and-operational-verification
  git checkout main

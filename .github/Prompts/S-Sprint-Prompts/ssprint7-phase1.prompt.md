...
---
mode: agent
description: S-Sprint-7 Phase 1 - Daily stale reconciliation
---

# S-Sprint-7 Phase 1 - Daily stale reconciliation

## Branching
1. Checkout `main` and pull latest from `origin/main`
2. Create `S-Sprint-7-base` from `main` if it does not already exist
3. Create `S-Sprint-7-base/phase-1-daily-stale-reconciliation` from `S-Sprint-7-base`
4. All work on `S-Sprint-7-base/phase-1-daily-stale-reconciliation`
5. Developer merges: `S-Sprint-7-base/phase-1-daily-stale-reconciliation` -> `S-Sprint-7-base` -> `main`

## Context
This is Phase 1 of S-Sprint-7 (Stale Reconciliation and Operational Hardening).

See `DropBox Synchronisation Using WebHooks.md` section for S-Sprint-7 for full phase architecture.

**Phase Brief:** Add the slower periodic stale-sync path that requests background sync for overdue synced projects even without recent webhook traffic.

## Reading List
Read the following files in order before writing code:

1. `DropBox Synchronisation Using WebHooks.md`
   - Read S-Sprint-7 Phase 1 section
...
### Phase Goal
Add the slower periodic stale-sync path that requests background sync for overdue synced projects even without recent webhook traffic.

### Key Deliverables
Refer to `DropBox Synchronisation Using WebHooks.md` S-Sprint-7 Phase 1 for:
...
- [ ] Committed to `S-Sprint-7-base/phase-1-daily-stale-reconciliation` with message:
    `feat: S-Sprint-7 Phase 1 - Daily stale reconciliation`
- [ ] `git status` is clean

**Gate 6 - TASKS.md updated**
- [ ] S-Sprint-7 Phase 1 marked complete in TASKS.md
- [ ] Committed with message: `chore: mark S-Sprint-7 Phase 1 complete in TASKS.md`

**Gate 7 - Present merge commands**
- [ ] Present for manual execution - do not execute:
  ```
  git checkout S-Sprint-7-base
  git merge S-Sprint-7-base/phase-1-daily-stale-reconciliation
  git checkout main
